...
void (*link_handler)(net_event_t) = NULL;
...
void net_init_link(uint32_t waitIP_timeout){
	...
	xTaskCreate(ethernet_link_thread, "ethernet_link_thread", stackdepth_kilobyte(ETHERNET_LINK_TASK_SIZE_KBYTE), &gnetif, ETHERNET_LINK_TASK_PRIORITY, NULL);
}
...
static void link_event_handler(struct netif *netif){
	if (netif_is_up(netif) || netif_is_link_up(netif)){
		...
		if(link_handler) link_handler(NET_EVENT_LINKUP);

		dhcp_start(&gnetif);
		/* Registers default network interface */
		netif_set_default(&gnetif);
		net_wait_ip_address();
	}
	else {
		...
		if(link_handler) link_handler(NET_EVENT_LINKDOWN);
	}
}
...
void net_wait_ip_address(void){
	...
	show_ip_address();
	if(link_handler) link_handler(NET_EVENT_GOTIP);
}
