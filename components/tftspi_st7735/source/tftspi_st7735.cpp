...
void TFT_ST7735::Init(spi_t spi, Color_Format Format) {
	_format = Format;
	_spi = spi;
...
void TFT_ST7735::DrawLine(uint16_t X1, uint16_t Y1, uint16_t X2, uint16_t Y2, uint16_t color) {
	int16_t dX = X2-X1;
	int16_t dY = Y2-Y1;
...
//		if (Y2 > Y1) ST7735_VLine(X1, Y1, Y2, color); else ST7735_VLine(X1, Y2, Y1, color);
		if (Y2 > Y1) FillRectangle(X1, Y1, X1, Y2, color); else FillRectangle(X1, Y2, X1, Y1, color);
		return;
	}
	if (dY == 0) {
//		if (X2>X1) ST7735_HLine(X1, X2, Y1, color); else ST7735_HLine(X2, X1, Y1, color);
		if (X2 > X1) FillRectangle(X1, Y1, X2, Y1, color); else FillRectangle(X2, Y1, X1, Y1, color);
		return;
	}
...
uint16_t TFT_ST7735::RGB565(uint8_t R, uint8_t G, uint8_t B) {
