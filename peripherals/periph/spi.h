...
class spi {
	public:
		spi(SPI_TypeDef *spi);
		result_t init(spi_config_t *conf);
		result_t deinit(void);

		result_t register_event_handler(void (*function_ptr)(spi_event_t event, void *param), void *param = NULL);
		spi_config_t *get_config(void);

		result_t transmit(uint32_t data, uint32_t size);
		result_t receive(uint32_t data, uint32_t size);
		result_t transmit_receive(uint32_t txdata, uint32_t rxdata, uint32_t size);


		result_t transmit_start_it(uint32_t data, uint32_t size);
		result_t receive_start_it(uint32_t data, uint32_t size);
		result_t transmit_receive_start_it(uint32_t txdata, uint32_t rxdata, uint32_t size);

		result_t transmit_stop_it(void);
		result_t receive_stop_it(void);
		result_t transmit_receive_stop_it(void);

#if ENABLE_DMA
		result_t transmit_start_dma(uint32_t data, uint32_t size);
		result_t receive_start_dma(uint32_t data, uint32_t size);
		result_t transmit_receive_start_dma(uint32_t txdata, uint32_t rxdata, uint32_t size);

		result_t transmit_stop_dma(void);
		result_t receive_stop_dma(void);
		result_t transmit_receive_stop_dma(void);
