...
class usart {
	public:
		...
		result_t init(usart_config_t *conf);
		result_t deinit(void);

		result_t register_event_handler(void (*function_ptr)(usart_event_t event, void *param), void *param = NULL);

		result_t transmit(uint8_t data);
		result_t transmit(uint8_t *data, uint16_t len);
		result_t send_string(char *string);

		result_t receive(uint16_t len);
		result_t receive(uint8_t *data);
		uint8_t  get_data(void);

		result_t transmit_start_it(uint8_t *data, uint16_t len);
		result_t receive_start_it(uint16_t buffer_size);
		result_t transmit_stop_it(void);
		result_t receive_stop_it(void);

#if ENABLE_DMA
		result_t transmit_start_dma(uint8_t *data, uint16_t len);
		result_t receive_start_dma(uint16_t len);
		result_t transmit_stop_dma(void);
		result_t receive_stop_dma(void);
#endif /* ENABLE_DMA */

		result_t receive_to_idle_start_it(uint16_t buffer_size);
		result_t receive_to_idle_stop_it(void);
#if ENABLE_DMA
		result_t receive_to_idle_start_it_dma(uint16_t buffer_size);
		result_t receive_to_idle_stop_it_dma(void);
#endif /* ENABLE_DMA */
		result_t receive_to_endchar_start_it(uint16_t buffer_size, char endchar = '\0');
		result_t receive_to_endchar_stop_it(void);
#if ENABLE_DMA
		result_t receive_to_endchar_start_dma(uint16_t buffer_size, char endchar = '\0');
		result_t receive_to_endchar_stop_dma(void);
#endif /* ENABLE_DMA */

		result_t get_buffer(uint8_t **data);
