...
result_t i2c::send_repeatstart(void){
	result_t ret;
...
result_t i2c::send_slaveaddress(uint16_t slaveaddress, i2c_action_t action){
	result_t ret;
...
result_t i2c::send_start_slaveaddress(uint16_t slaveaddress, i2c_action_t action){
	result_t ret;
...
result_t i2c::send_repeatstart_slaveaddress(uint16_t slaveaddress, i2c_action_t action){
	result_t ret;
...
result_t i2c::check_device(uint16_t slaveaddress, uint8_t trials, uint16_t timeout){
	result_t ret;
...
result_t i2c::transmit(uint8_t *data, uint16_t size){
	result_t ret;
...
result_t i2c::transmit(uint8_t data){
	result_t ret;
	/* ***********************WAIT TXE FLAG IS SET*********************** */
	ret = wait_check_flag_in_register_timeout(&(_i2c -> SR1), I2C_SR1_AF, FLAG_SET, &(_i2c -> SR1), I2C_SR1_TXE, FLAG_SET, I2C_TIMEOUT);
	if(!err_is_oke(&ret)){
	...
	while(RxCount > 0U){
		if(RxCount <= 3U){
			if(RxCount == 1U){
				...
				if(!err_is_oke(&ret)){
					if(err_is_fail(&ret)) _i2c -> SR1 = ~(I2C_SR1_STOPF & 0xFFFF);
					set_return_line(&ret, __LINE__);
					return ret;
				}
				// READ FORM DR.
...
#if ENABLE_DMA
result_t i2c::transmit_start_dma(uint8_t *data, uint16_t size){
	result_t ret;
...
result_t i2c::receive_start_dma(uint8_t *data, uint16_t size){
	result_t ret;
...
result_t i2c::transmit_stop_dma(void){
	result_t ret;
...
result_t i2c::receive_stop_dma(void){
	result_t ret;
...
result_t i2c::i2c_write_register(uint16_t slaveaddress, uint16_t registeraddress, uint8_t registeraddress_size, uint8_t *data, uint16_t size){
	result_t ret;
...
result_t i2c::i2c_read_register(uint16_t slaveaddress, uint16_t registeraddress, uint8_t registeraddress_size, uint8_t *data, uint16_t size){
	result_t ret;
