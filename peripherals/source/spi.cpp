...
result_t cc;
result_t SPI_Tx_ISR(spi *spi);
result_t SPI_Rx_ISR(spi *spi);
...
result_t spi::init(spi_config_t *conf){
	result_t ret;
	_conf = conf;
#if ENABLE_DMA
	_txdma = _conf->txdma;
...
result_t spi::register_event_handler(void (*function_ptr)(spi_event_t event, void *param), void *param){
	result_t ret;
	if(_conf -> control && (SPI_INTERRUPT_CONTROL | SPI_INTERRUPT_DMA_CONTROL)) {
...
result_t spi::transmit(uint32_t data, uint32_t size){
	result_t ret;
...
result_t spi::receive(uint32_t data, uint32_t size){
	result_t ret;
...
result_t spi::transmit_receive(uint32_t txdata, uint32_t rxdata, uint32_t size){
	result_t ret;
...
result_t spi::transmit_start_it(uint32_t data, uint32_t size){
	result_t ret;
...
result_t spi::receive_start_it(uint32_t data, uint32_t size){
	result_t ret;
...
result_t spi::transmit_receive_start_it(uint32_t txdata, uint32_t rxdata, uint32_t size){
	result_t ret;
...
#if ENABLE_DMA
result_t spi::transmit_start_dma(uint32_t data, uint32_t size){
	result_t ret;
	txcount = 0U;
	txlen = size;
	txbuf = data;
...
result_t spi::receive_start_dma(uint32_t data, uint32_t size){
	result_t ret;
...
result_t spi::transmit_receive_start_dma(uint32_t txdata, uint32_t rxdata, uint32_t size){
	result_t ret;
...
result_t spi::transmit_stop_dma(void){
	result_t ret;
...
result_t spi::receive_stop_dma(void){
	result_t ret;
...
result_t spi::transmit_receive_stop_dma(void){
	result_t ret;
...
result_t SPI_Tx_ISR(spi *spi){
	...
	if(spi -> txcount == spi -> txlen){
		result_t ret = wait_flag_in_register_timeout(&(spi->_spi->SR), SPI_SR_TXE, FLAG_SET, SPI_TIMEOUT);
		if(!err_is_oke(&ret)) {
			set_return_line(&ret, __LINE__);
#if CONFIG_USE_LOG_MONITOR && SPI_LOG
			LOG_ERROR(TAG, "%s -> %s -> Wait flag timeout.", __FILE__, __FUNCTION__);
...
result_t SPI_Rx_ISR(spi *spi){
	...
	*(uint32_t *)(spi -> rxbuf) = spi -> _spi -> DR ;
	...
	if(spi -> rxcount == spi -> rxlen){
		...
		result_t ret = wait_flag_in_register_timeout(&(spi->_spi->SR), SPI_SR_RXNE, FLAG_RESET, SPI_TIMEOUT);
		if(!err_is_oke(&ret)) {
#if CONFIG_USE_LOG_MONITOR && SPI_LOG
			LOG_ERROR(TAG, "%s -> %s -> Wait flag timeout.", __FILE__, __FUNCTION__);
#endif /* CONFIG_USE_LOG_MONITOR && SPI_LOG */
