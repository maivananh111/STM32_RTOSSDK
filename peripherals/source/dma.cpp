...
result_t dma::start(uint32_t Src_Address, uint32_t Dest_Address, uint32_t Number_Data){
	...
	if(_state == E_READY){
		...
		if(err_is_timeout(&ret)){
#if CONFIG_USE_LOG_MONITOR && DMA_LOG
			E_LOGE(TAG, "%s -> %s -> Wait flag timeout.", __FILE__, __FUNCTION__);
#endif /* CONFIG_USE_LOG_MONITOR && DMA_LOG */
			set_return_line(&ret, __LINE__);
			return ret;
		}
...
result_t dma::poll_for_tranfer(dma_interruptoption_t PollLevel, uint32_t TimeOut){
	result_t ret;
	...
	while(!(isr & PollValue)){
		if(get_tick() - tick > TimeOut){
			set_return(&ret, E_TIMEOUT, __LINE__);
			return ret;
		}
